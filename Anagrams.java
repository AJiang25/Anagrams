 ...
 * Anagrams. The second feature (most of the code) involves the code necessary
 * for GUI (Graphical User Interface) to emerge through the use of JButtons to
 * allow user-interactions in Anagrams (selecting letters) and printing the
 * current word (through the use of a JLabel). The third feature builds/add on
 * to the game by adding a shuffle JButton button similar to the iconic
 * Pigeon-game Anagrams (shuffle method, JButton in construction). It shuffles
 * the position of the x coordinates of the letter's JButton by either reversing
 * their order, shifting them to the right by one, and ... left by one through
 * a void method.
 * */
